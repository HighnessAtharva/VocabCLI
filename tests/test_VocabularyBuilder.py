...
class TestList:
    ...
    def test_list_words_in_collection_nonexistent(self):
...
class TestGraph:
    class Test_Top_Words_Bar_Graph:
        ...
        def test_graph_bar_top_words(self): 
            runner.invoke(app, ["define", "math", "rock", "class","math","red","colour","traffic","stuck","happy"])
            result = runner.invoke(app,["graph","--topwordsbar"])
            assert result.exit_code == 0
    ...
    class Test_Top_Words_Pie_Chart:
        ...
        def test_graph_top_words_less_than_N(self):
            ...
            result = runner.invoke(app,["graph","--topwordspie"])
            assert result.exit_code == 0
            assert "Not enough words found. Showing Pie Chart for available words only" in result.stdout
    
    class Test_Top_Tags_Bar_Graph:
        def test_graph_top_tags_bar(self):
            runner.invoke(app, ["define", "math", "rock", "class","red","colour","traffic","stuck","happy","sad","rain"])
            ...
            result = runner.invoke(app,["graph","--toptagsbar"])
            assert result.exit_code == 0
            assert "Displaying Bar graph of top" in result.stdout
